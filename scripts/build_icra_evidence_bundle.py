...
class EvidenceInputs:
    ...
    additional_sources: tuple[Path, ...] = field(default_factory=tuple)
    statistics_summary: Path | None = None
    ...
    def source_paths(self) -> tuple[Path, ...]:
        return self.metric_paths() + self.protocols + self.additional_sources + ((self.statistics_summary,) if self.statistics_summary else ()) + ((self.reference_metrics,) if self.reference_metrics else ())
    ...
    *,
    ...
    for row in rows:
        if int(row["seed"]) not in EXPECTED_SEEDS:
            raise ValueError(f"{source}: unexpected seed {row['seed']} (expected {EXPECTED_SEEDS})")
    ...
    dedicated_groups: dict[tuple[str, str, str], list[dict[str, object]]] = defaultdict(list)
    for row in rows:
        if str(row.get("replicate_kind", "")) == "dedicated_run":
            dedicated_groups[(str(row["method"]), str(row["traversal"]), str(row["profile"]))].append(row)
    for group_key, group in dedicated_groups.items():
        if len(group) != 3 or {int(row["seed"]) for row in group} != set(EXPECTED_SEEDS):
            raise ValueError(f"{source}: dedicated run group {group_key} must contain seeds {EXPECTED_SEEDS}")
        hashes = [str(row.get("source_output_sha256", "")) for row in group]
        if all(hashes) and len(set(hashes)) != len(hashes):
            raise ValueError(f"{source}: dedicated run group {group_key} reuses an output hash")
    if len(commits) > 1:
...
def _tex_main_table(path: Path, rows: list[dict[str, object]]) -> None:
    ...
    for traversal in EXPECTED_TRAVERSALS:
        ...
        for method in method_order:
            ...
            row_corr = format_mean_std(vals.get("inrow_row_correct_fraction", [row.get("inrow_row_correct_fraction_mean", row.get("row_correct_fraction_mean"))]), digits=3)
...
def _tex_gnss_stress_table(path: Path, gnss_stress_csv: Path | None) -> None:
    ...
    if gnss_stress_csv and gnss_stress_csv.exists():
        ...
        if compact_csv.exists():
            ...
            profiles = ["nominal", "outage_5s", "outage_10s", "outage_20s", "drift_bias", "gaussian_high", "multipath_bursts", "headland_only_degradation"]
            profile_labels = {
                "nominal": "Nominal",
                ...
                "headland_only_degradation": "Headland-Only",
            }
...
def build_bundle(inputs: EvidenceInputs, output_dir: Path) -> dict[str, object]:
    ...
    for path in inputs.baseline_metrics:
        ...
        _validate_rows(b_rows, path, require_complete_matrix=False)
    ...
    if inputs.statistics_summary:
        statistics_data = json.loads(inputs.statistics_summary.read_text(encoding="utf-8"))
        if statistics_data.get("schema_version") != 1:
            raise ValueError(f"{inputs.statistics_summary}: unsupported statistics schema")
    ...
    claim_checks = {
        ...
        "notes": [
            ...
            "SemanticBLT metadata is external model/dataset provenance only; it does not permit current-field, per-class, or confusion-matrix detector claims.",
        ],
    }
    ...
    if inputs.statistics_summary:
        outputs["statistics_summary"] = str(inputs.statistics_summary)
    manifest: dict[str, object] = {
        ...
        "inputs": {
            ...
            "additional_sources": [str(path) for path in inputs.additional_sources],
            "statistics_summary": str(inputs.statistics_summary) if inputs.statistics_summary else None,
        ...
        "outputs": outputs,
        "table_registry": {
            "main": {"generated": table_paths["main"], "source_csv": [str(inputs.followup_metrics), *[str(p) for p in inputs.baseline_metrics]], "row_key": "method,traversal,seed,profile"},
            "operational": {"generated": table_paths["operational"], "source_csv": [str(inputs.followup_metrics)], "row_key": "method,traversal,seed,profile"},
            "gnss_stress": {"generated": table_paths["gnss_stress"], "source_csv": [str(inputs.gnss_stress_metrics)] if inputs.gnss_stress_metrics else [], "row_key": "method,traversal,profile,seed"},
            "ablation": {"generated": table_paths["ablation"], "source_csv": [str(inputs.ablation_metrics)] if inputs.ablation_metrics else [], "row_key": "variant,traversal,seed"},
            "robustness": {"generated": table_paths["robustness"], "source_csv": [str(inputs.robustness_metrics)] if inputs.robustness_metrics else [], "row_key": "option,variant,traversal,seed"},
        },
...
def main() -> int:
    ...
    parser.add_argument("--additional-source", action="append", type=Path, default=[])
    parser.add_argument("--statistics", type=Path, default=None, help="Machine-readable statistics summary JSON")
    ...
    manifest = build_bundle(
        EvidenceInputs(
            ...
            statistics_summary=args.statistics,
