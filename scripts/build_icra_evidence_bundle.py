        ...
        for row in rows:
            ...
            for key, _ in columns:
                if key in {"method", "traversal"}:
                    value = str(row.get(key, "")).replace("&", r"\&").replace("_", r"\_")
                    cells.append(r"\texttt{" + value + "}")
                else:
...
def _write_tables(rows: list[dict[str, object]], output_dir: Path) -> dict[str, str]:
    ...
    columns = {
        "main": (("method", "Method"), ("traversal", "Traversal"), ("ape_raw_rmse", "Raw APE (m)"), ("ape_align_rmse", "Aligned APE (m)"), ("rpe_2m_align_rmse", "RPE 2m (m)")),
        "gnss_stress": (("method", "Method"), ("traversal", "Traversal"), ("ape_raw_rmse", "Raw APE (m)"), ("ape_align_rmse", "Aligned APE (m)"), ("rpe_5m_align_rmse", "RPE 5m (m)")),
        "operational": (("method", "Method"), ("traversal", "Traversal"), ("inrow_cross_track_mean", "In-row XT (m)"), ("inrow_row_correct_fraction", "In-row correct"), ("inrow_wrong_row_duration_sec", "In-row wrong (s)"), ("headland_cross_track_mean", "Headland XT (m)"), ("headland_mean_recovery_distance_m", "Headland recovery (m)")),
    }
    grouped_rows = {"main": main, "gnss_stress": stress, "operational": operational}
    paths = {
    ...
    for key, path in paths.items():
        _tex_table(path, grouped_rows[key], columns[key])
        _tex_table(icra_paths[key], grouped_rows[key], columns[key])
        paths[key] = icra_paths[key]
